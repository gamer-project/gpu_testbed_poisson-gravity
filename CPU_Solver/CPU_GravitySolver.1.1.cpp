...
//-----------------------------------------------------------------------------------------
// Function    :  CPU_GravitySolver
// Description :  Use CPU to advance the momentum and energy density by gravitational acceleration
//
// Parameter   :  Flu_Array   : The array to store the input and output fluid variables
//                Pot_Array   : The array to store the input potential for evaluating the acceleration
//                NPatchGroup : The number of patch groups to be evaluated
//                dt          : The time interval to advance solution
//                dh          : Grid size
...
//       advance fluid
         rho  = Flu_Array[P][0][kk][jj][ii];
         px   = Flu_Array[P][1][kk][jj][ii];
         py   = Flu_Array[P][2][kk][jj][ii];
         pz   = Flu_Array[P][3][kk][jj][ii];
         temp = (real)0.5/rho;
         Eint = Flu_Array[P][4][kk][jj][ii] - temp*(px*px+py*py+pz*pz);

         px -= rho*Acc[0];
         py -= rho*Acc[1];
         pz -= rho*Acc[2];

         Flu_Array[P][1][kk][jj][ii] = px;
         Flu_Array[P][2][kk][jj][ii] = py;
         Flu_Array[P][3][kk][jj][ii] = pz;
         Flu_Array[P][4][kk][jj][ii] = Eint + temp*(px*px+py*py+pz*pz);
