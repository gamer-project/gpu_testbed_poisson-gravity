...
//
// Parameter   :  g_Flu_Array : The global memory array to store the input and output fluid variables
//                g_Pot_Array : The global memory array to store the input potential for evaluating the 
//                              acceleration
...
//    advance the fluid
      rho  = g_Flu_Array[bx][0][g_index];
      px   = g_Flu_Array[bx][1][g_index];
      py   = g_Flu_Array[bx][2][g_index];
      pz   = g_Flu_Array[bx][3][g_index];
      temp = (real)0.5/rho;
      Eint = g_Flu_Array[bx][4][g_index] - temp*(px*px+py*py+pz*pz);
      
      px -= rho*Acc[0];
      py -= rho*Acc[1];
      pz -= rho*Acc[2];
      
      g_Flu_Array[bx][1][g_index] = px;
      g_Flu_Array[bx][2][g_index] = py;
      g_Flu_Array[bx][3][g_index] = pz;
      g_Flu_Array[bx][4][g_index] = Eint + temp*(px*px+py*py+pz*pz);
